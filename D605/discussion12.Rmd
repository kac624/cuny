...
```{r}
df <- df %>%
  mutate(children = children + 0.01)

for (predictor in c('age','bmi','children')) {
  if (predictor == 'charges') next
  formula <- as.formula(paste(predictor,' ~ 1'))
  transformation <- powerTransform(formula, data = df)
  transformed <- bcPower(df[predictor], transformation$lambda)
  df[predictor] <- transformed
}
