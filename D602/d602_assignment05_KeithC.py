...
def exercise11(n):
    ...
    try:
        ...
        array_reshaped = 'Unable to reshape. Input n must be divisible by 3.'
    # ------ Place code above here /\ /\ /\ ------  
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.DataFrame(np.random.randint(0, n, n), index = pd.date_range('1/1/2010', periods = n, freq = 'D'))
    fig, ax = plt.subplots()
    ax.plot(s.cumsum())
    plt.show()
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame({'letters':[len(x) for x in words]})
    # ------ Place code above here /\ /\ /\ ------ 
    return df
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.read_csv('https://tinyurl.com/y63q7okz').iloc[::5,0:3:2]
    # ------ Place code above here /\ /\ /\ ------ 
    return df
