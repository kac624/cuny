...
```{r}
model_vars <- colnames(evals)
exclusions <- c()
results <- list()
counter <- 0

for (i in 1:length(evals)) {
  model <- lm(score ~ ., data = select(evals, -all_of(exclusions)))
  worst_pval <- names(sort(summary(model)$coefficients[,4], 
                           decreasing = TRUE))[1]
  worst_pval <- str_sub(worst_pval,1,7)
  worst_pval <- evals %>% 
    select(contains(worst_pval)) %>% 
    colnames
  exclusions <- c(exclusions, worst_pval)
  results[[i]] <- list(vars = colnames(model$model),
