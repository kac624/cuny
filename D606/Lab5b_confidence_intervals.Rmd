...
The proportion of CIs that capture the population proportion is 48 out of 50 or 96%, roughly conforming to the confidence level, as expected. The match, however, is not exact. First, we don't have 100 confidence intervals, and second, there is a degree of randomness to these simulations, so we can't expect that results exactly conform to expected values.
...
I've reperformed the sample here and compared the original 95% CI to a 99% CI. 
...
I'll use an 80% confidence interval,
...
```{r}
p1 <- plot_ci(sample_size = 60, conf_level = 0.95, iterations = 50)
p2 <- plot_ci(sample_size = 10, conf_level = 0.95, iterations = 50)
