...
scores %>%
  mutate(score_norm_abs_rank = min_rank(score_norm_abs)) %>%
  ggplot(aes(score_norm_abs_rank)) +
  geom_histogram()
```

## Additional Sources

Vote View NOMINATE Ideology scores
...
candidates <- candidates %>%
  left_join(select(scores, name, score_norm_abs, ecdf_rank)) %>%
  ...
  rename(partisan_score_twitter = ecdf_rank,
         # partisan_score_twitter = score_norm_abs,
...
candidates %>%
  ...
  filter(dupe == FALSE) %>%
  ggplot() +
  geom_freqpoly(aes(partisan_score_twitter), color = 'blue') +
  geom_freqpoly(aes(partisan_score_nominate), color = 'green') +
  geom_freqpoly(aes(partisan_score_govtrack), color = 'red')
```
