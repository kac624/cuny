...
candidates <- candidates %>%
  left_join(select(scores, name, score_norm_abs)) %>%
  ...
  rename(partisan_score_twitter = score_norm_abs,
...
candidates %>%
  mutate(dupe = duplicated(name)) %>%
  filter(!is.na(partisan_score_twitter), 
         !is.na(partisan_score_nominate), 
         !is.na(partisan_score_govtrack),
         dupe == FALSE) %>%
  ggplot() +
