...
tweets <- read_csv(unzip(temp, 'candidate_tweets2022.csv'))
unlink(temp)
```
...
tweets_afinn %>%
  ...
  ggplot(aes(hour, sentiment)) +
  geom_col(show.legend = FALSE)

tweets_afinn %>%
  left_join(select(candidates, party, twitter_id), 
            by = 'twitter_id') %>%
  left_join(select(tweets, likes, twitter_id),
            by = 'twitter_id') %>%
  ggplot(aes(sentiment, likes, color = source)) +
