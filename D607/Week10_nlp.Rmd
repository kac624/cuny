...
```{r}
username <- 'kac624'
authkey <- '685bf57c27bb46400dc2d8e93d514838'

url <- 'https://www.kaggle.com/api/v1/datasets/download/kac624/moc2020tweets/tweets2020.csv'
response <- GET(url, authenticate(username, authkey, type = "basic"))
temp <- tempfile()
download.file(response$url,temp)
tweets <- read.csv(unzip(temp, "tweets2020.csv"))
unlink(temp)
```

```{r}
tweets <- tweets %>%
  select(-views, -retweeted_id, -quoted_id) %>%
  mutate(date_created = str_extract(date_created, '.*(?=\\+00\\:00)'),
...
tweets_afinn <- tidy_tweets %>% 
  ...
  group_by(tweet_id, date_created, username, verified_status,
           likes, replies, retweets, source, language) %>% 
...
tweets_afinn %>%
  mutate(hour = lubridate::hour(date_created)) %>%
  group_by(hour, sentiment) %>%
  ggplot(aes(hour, sentiment, fill = method)) +
  geom_col(show.legend = FALSE)

tweets_afinn %>%
  ggplot(aes(sentiment, replies, color = source)) +
