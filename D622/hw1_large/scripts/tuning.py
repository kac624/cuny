...
hp_space = {
    ...
    'PRETRAINED_LM': ['bert-base-uncased'], # 'bert-large-uncased', 'bert-large-cased'
    'LOGGING': [False],
    'MAX_LENGTH': [128], # 32, 64, 128, 256
    'BATCH_SIZE': [32], # 32, 64, 128
    'NUM_EPOCHS': [10],
    'LEARNING_RATE': [2e-5], # 1e-4, 2e-5, 2e-6
    'DROPOUT': [0.3], # 0.1, 0.3, 0.5
    'BALANCE_FACTOR': [0.3], # 0.1, 0.3, 0.5
    'PATIENCE': [3]
}
...
for counter, values in enumerate(hp_combos):
    ...
    # Run training
    print('\n----TRAINING----')
    subprocess.run(['python', 'scripts/train_bert.py'])
