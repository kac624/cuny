...
input_ids_train = torch.load('data/processed/train_input_ids.pt')
att_masks_train = torch.load('data/processed/train_att_masks.pt')
y_train = torch.load('data/processed/y_train.pt')

input_ids_valid = torch.load('data/processed/valid_input_ids.pt')
att_masks_valid = torch.load('data/processed/valid_att_masks.pt')
y_valid = torch.load('data/processed/y_valid.pt')

input_ids_test = torch.load('data/processed/test_input_ids.pt')
att_masks_test = torch.load('data/processed/test_att_masks.pt')
y_test = torch.load('data/processed/y_test.pt')

with open('data/processed/code_labels_dict.json', 'r') as f:
...
test_class_report = classification_report(test_pred, y_test, output_dict=True, target_names=label_names, zero_division=0)

# Create summary df
summary = pd.DataFrame(config, index=[log_stamp]).transpose()
# Loop through classification reports to add to summary
for report, name in zip([train_class_report, valid_class_report, test_class_report],['train', 'valid', 'test']):
    temp = pd.DataFrame({
...
if LOGGING:
    ...
    test_output = pd.DataFrame({'test_pred': test_pred, 'y_test': y_test})

    train_class_report = pd.DataFrame(train_class_report).transpose()
    valid_class_report = pd.DataFrame(valid_class_report).transpose()
    test_class_report = pd.DataFrame(test_class_report).transpose()

    # Log to excel                    
    filename = f'logs/{log_stamp}.xlsx'
    ...
    with pd.ExcelWriter(filename, mode='w', engine='openpyxl') as writer:
        ...
        test_class_report.to_excel(writer, sheet_name='test_class_report')
        ...
        test_output.to_excel(writer, sheet_name='test_results')
else:
